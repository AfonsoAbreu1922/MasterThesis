class LUNA25PreprocessedDataLoader(Dataset):
    def __init__(
        ...
        self.image_transformer = torchvision.transforms.Compose([
            lambda x: numpy.transpose(x, axes=(1, 2, 0)) if x.ndim == 3 else x,
    ...
    def __getitem__(self, data_index):
        image = numpy.load(f"{self.image_numpy_arrays_dir_path}/{self.file_names[data_index]}.npy").astype(numpy.float32)
