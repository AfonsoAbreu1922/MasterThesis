...
class ModelTrainingPerformanceMetricsFigure:
    ...
    def add_loss_related_traces(
        ...
        for df_column_name, trace_name, trace_line_colour in zip(
            ...
            for data_fold_id \
                    in range(1, self.config.number_of_data_folds + 1):
            ...
            if self.config.number_of_data_folds > 1:
                ...
                self.figure.add_trace(
                    ...
                    row=self.config.number_of_data_folds + 1,
                    col=trace_column_number
                )
                self.figure.add_trace(
                    plotly.graph_objects.Scatter(
    ...
    def add_validation_performance_metrics_traces(
        ...
        for df_column_name, trace_name, trace_line_colour in zip(
            ...
            for data_fold_id in range(1, self.config.number_of_data_folds + 1):
                self.figure.add_trace(
                    plotly.graph_objects.Scatter(
                        y=performance_metrics_dataframes[
                            f'datafold_{data_fold_id}'
    ...
    def get_axis_max_values(self):
        ...
        train_and_validation_loss_y_axis_max_values = {
            y_axis_idx: math.ceil(
                ...
                range(1, self.config.number_of_data_folds + 1),
                range(1, 2 * (self.config.number_of_data_folds + 1), 2)
            )
        }
    ...
    def set(self):
        self.figure = make_subplots(
            rows=self.config.number_of_data_folds + 1,
            cols=2,
            vertical_spacing=0.075,
            subplot_titles=(
                "<b>Fig. 1 - Loss X Epochs</b>",
                "<b>Fig. 2 - LNM score prediction"
                " performance metrics X Epochs</b>"
            )
        )
    ...
    def update_layout(self):
        ...
        self.figure.update_layout(
            ...
            **{
                ...
                for xaxis_idx
                in range(1, 2 * (self.config.number_of_data_folds + 1) + 1)
            },
            yaxis=dict(
                side="left",
            ...
            **{
                ...
                in range(2, 2 * (self.config.number_of_data_folds + 1), 2)
            }
        )
